...
exports.authenticatedUser = (req, res, next) => {
	if (!req.user) {
		req.flash('danger', "You're not currently logged in.");
		res.redirect('/');
	} else {
		next();
	}
...
exports.authenticatedUserWithDynamicRedirect = (req, res, next) => {
	if (!req.user) {
...
exports.admin = (req, res, next) => {
	if (!req.user.admin) {
		req.flash('danger', 'You need to be an admin to do that action.');
		res.redirect('/');
	} else {
		next();
	}
